  ...
  SUBROUTINE ELASTICITY_PRE_SOLVE(CONTROL_LOOP,SOLVER,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(CONTROL_LOOP%PROBLEM)) THEN
      ...
      CASE(EQUATIONS_SET_LINEAR_ELASTICITY_TYPE)
        !Do Nothing
      CASE(EQUATIONS_SET_FINITE_ELASTICITY_TYPE)
        !Need a problem type lable here to decide if a re-projection is required
        !IF(ASSOCIATED(SOLVER%SOLVER_EQUATIONS%SOLVER_MAPPING%INTERFACE_CONDITIONs(1)%PTR)) THEN !If it's an interface example
        !  INTERFACE_CONDITION=>SOLVER%SOLVER_EQUATIONS%SOLVER_MAPPING%INTERFACE_CONDITIONs(1)%PTR
        !  IF(INTERFACE_CONDITION%OPERATOR==INTERFACE_CONDITION_FRICTIONLESS_CONTACT_OPERATOR) THEN !Frictionless contact
        !    IF(INTERFACE_CONDITION%LAGRANGE%LAGRANGE_FIELD%VARIABLES(1)%COMPONENTS(1)%INTERPOLATION_TYPE== &
        !        & FIELD_DATA_POINT_BASED_INTERPOLATION) THEN !point connecitivity
              !Re-projection for data points connectivity frictionless contact mechanics.
        !      DO equationSetIdx=1,INTERFACE_CONDITION%DEPENDENT%NUMBER_OF_DEPENDENT_VARIABLES
                !dependent field to be projected on from the data points.
        !        DEPENDENT_FIELD=>INTERFACE_CONDITION%DEPENDENT%EQUATIONS_SETS(equationSetIdx)%PTR%DEPENDENT%DEPENDENT_FIELD
