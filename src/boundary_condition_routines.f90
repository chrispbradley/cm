...
MODULE BOUNDARY_CONDITIONS_ROUTINES
  ...
  PUBLIC BOUNDARY_CONDITIONS_SET_CONSTANT,BOUNDARY_CONDITIONS_SET_LOCAL_DOF,BOUNDARY_CONDITIONS_SET_ELEMENT, &
    ...
    & BOUNDARY_CONDITIONS_FACE_BASIS_PRESSURE_POISSON_CALCULATE,BOUNDARY_CONDITIONS_SET_DATA_POINT !,BOUNDARY_CONDITIONS_LINE_BASIS_PRESSURE_POISSON_CALCULATE
...
CONTAINS  
  ...
  SUBROUTINE BOUNDARY_CONDITIONS_CREATE_FINISH(BOUNDARY_CONDITIONS,ERR,ERROR,*)
    ...
    INTEGER(INTG) :: col_idx,equations_set_idx,bc_idx,parameterSetIdx,equations_matrix_idx !,interface_condition_idx,interface_matrix_idx
...
999 CALL ERRORS("BOUNDARY_CONDITIONS_SET_LOCAL_DOFS",ERR,ERROR)
  ...
  SUBROUTINE BOUNDARY_CONDITIONS_SET_DATA_POINT(BOUNDARY_CONDITIONS,FIELD,VARIABLE_TYPE,USER_DATA_POINT_NUMBER,COMPONENT_NUMBER, &
    ...
    TYPE(FIELD_TYPE), POINTER :: FIELD !<The dependent field to set the boundary condition on.
    INTEGER(INTG), INTENT(IN) :: VARIABLE_TYPE !<The variable type to set the boundary condition at
    INTEGER(INTG), INTENT(IN) :: USER_DATA_POINT_NUMBER !<The user element number to set the boundary condition at
    INTEGER(INTG), INTENT(IN) :: COMPONENT_NUMBER !<The component number to set the boundary condition at
    INTEGER(INTG), INTENT(IN) :: CONDITION !<The boundary condition type to set \see BOUNDARY_CONDITIONS_ROUTINES_BoundaryConditions,BOUNDARY_CONDITIONS_ROUTINES
    REAL(DP), INTENT(IN) :: VALUE !<The value of the boundary condition to set
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    ...
    CALL ENTERS("BOUNDARY_CONDITIONS_SET_DATA_POINT",ERR,ERROR,*999)
    ...
    IF(ASSOCIATED(BOUNDARY_CONDITIONS)) THEN
      ...
      ELSE
        IF(ASSOCIATED(FIELD)) THEN
          CALL FIELD_COMPONENT_DOF_GET_USER_DATA_POINT(FIELD,VARIABLE_TYPE,USER_DATA_POINT_NUMBER,COMPONENT_NUMBER, &
    ...
    CALL EXITS("BOUNDARY_CONDITION_SET_DATA_POINT")
    RETURN
999 CALL ERRORS("BOUNDARY_CONDITION_SET_DATA_POINT",ERR,ERROR)
    CALL EXITS("BOUNDARY_CONDITION_SET_DATA_POINT")
    RETURN 1
  END SUBROUTINE BOUNDARY_CONDITIONS_SET_DATA_POINT
